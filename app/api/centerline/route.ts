...
async function fetchTileFromPointer(origin: string, tileName: string): Promise<FeatureCollection | null> {
  ...
  // pointer text is tiny: cache it hard
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      ...
      {
        headers: {
          // cache the API response briefly; tiles themselves are force-cached
          "Cache-Control": "public, max-age=30",
        },
      }
    );
  } catch (err: any) {
    return NextResponse.json({ error: err?.message ?? "centerline error" }, { status: 500 });
  }
}
